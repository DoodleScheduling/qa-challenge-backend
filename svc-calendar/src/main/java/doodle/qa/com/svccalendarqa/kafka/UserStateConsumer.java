...
/**
 * Kafka consumer for user state events. Listens to the user-state topic and processes
 * user state events. Only saves user_id and calendar_id to the user_calendar table.
 */
@Component
@Slf4j
...
public class UserStateConsumer {
    ...
    /**
     * Handles user created or updated events.
     * Only saves user_id and calendar_id to the user_calendar table.
     *
     * @param userState the user state event
     */
    private void handleUserCreatedOrUpdated(UserState userState) {
        ...
        if (!userState.getCalendarIds().isEmpty()) {
            ...
            calendarIds = userState.getCalendarIds().stream()
                    .map(UUID::fromString)
                    .collect(Collectors.toList());

            log.info("Processing user created/updated event for user ID: {} with {} calendar(s)", userId, calendarIds.size());
        } else {
     ...
     * Only deletes entries from the user_calendar table.
     *
    ...
    /**
     * Handles calendar added or removed events.
     * Only updates entries in the user_calendar table.
     *
     * @param userState the user state event
     */
    private void handleCalendarChange(UserState userState) {
        ...
        log.info("Processing calendar change event for user ID: {} with event type: {} and {} calendar(s)", 
                userId, eventType, calendarIds.size());
