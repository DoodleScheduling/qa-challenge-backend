...
import doodle.qa.com.svccalendarqa.service.CalendarService;
...
public class CalendarController {
   ...
   * @return List of calendars owned by the user
   */
  @GetMapping("/user/{userId}")
  @Operation(
      summary = "Get calendars by user ID",
      description = "Retrieves all calendars owned by a specific user")
  @ApiResponse(responseCode = "200", description = "Calendars retrieved successfully")
  @ApiResponse(responseCode = "404", description = "User not found", content = @Content)
  public ResponseEntity<List<CalendarDto>> getCalendarsByUserId(
      @Parameter(description = "User ID", required = true) @PathVariable UUID userId) {
    log.debug("GET request to retrieve calendars for user: {}", userId);
    List<CalendarDto> calendars = calendarService.getCalendarsByUserId(userId);
    log.info("Retrieved {} calendars for user: {}", calendars.size(), userId);
  ...
  public ResponseEntity<CalendarDto> createCalendar(
    ...
    log.debug("POST request to create calendar: {}", calendarDto.getName());
