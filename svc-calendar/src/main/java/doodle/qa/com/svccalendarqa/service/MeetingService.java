...
public class MeetingService {
    ...
    public Page<Meeting> findMeetings(
        ...
        // Find meetings
        return meetingRepository.findByUserCalendarAndStartTimeGreaterThanEqualAndEndTimeLessThanEqualOrderByStartTimeAsc(
    ...
    public List<TimeSlotDto> findAvailableTimeSlots(
        ...
        if (slotDurationMinutes > MAX_SLOT_DURATION_HOURS * 60) {
            throw new IllegalArgumentException("Slot duration cannot exceed " + MAX_SLOT_DURATION_HOURS + " hours");
        }

        // Get busy slots from meetings
        List<Meeting> busyMeetings = meetingRepository.findOverlappingMeetingsByUserCalendar(userCalendar, from, to);
        ...
        for (Meeting meeting : busyMeetings) {
            busySlots.add(TimeSlotDto.builder()
                    ...
                    .durationMinutes((int) ChronoUnit.MINUTES.between(meeting.getStartTime(), meeting.getEndTime()))
                    .build());
        }
    ...
    public Meeting findMeeting(
        ...
        // Find meeting
        return meetingRepository.findByUserCalendarAndId(userCalendar, meetingId)
    ...
    public Meeting createMeeting(
            @Valid @NotNull MeetingDto meetingDto,
            @NotNull UUID userId) {
    ...
    public Meeting updateMeeting(
        ...
        // Find meeting
        Meeting meeting = meetingRepository.findByUserCalendarAndId(userCalendar, meetingId)
                .orElseThrow(() -> new MeetingNotFoundException(meetingId, userId, calendarId));
    ...
    private UserCalendar validateUserAndCalendar(UUID userId, UUID calendarId) {
        return userCalendarRepository.findByCalendarIdAndUserId(calendarId, userId)
    ...
    private void validateTimeRange(LocalDateTime from, LocalDateTime to) {
        ...
        if (daysBetween > MAX_TIME_RANGE_DAYS) {
            throw new IllegalArgumentException("Time range cannot exceed " + MAX_TIME_RANGE_DAYS + " days");
        }
    }

    /**
     * Validate that the meeting time is valid and the duration does not exceed the maximum allowed duration.
     *
    ...
    private void validateMeetingTime(LocalDateTime startTime, LocalDateTime endTime) {
        ...
        if (hoursBetween > MAX_SLOT_DURATION_HOURS) {
            throw new IllegalArgumentException("Meeting duration cannot exceed " + MAX_SLOT_DURATION_HOURS + " hours");
        }
    }
     ...
     */
    private void checkForConflicts(UUID calendarId, LocalDateTime startTime, LocalDateTime endTime, UUID excludeMeetingId) {
        // Get the UserCalendar for this calendar ID
        UserCalendar userCalendar = userCalendarRepository.findByCalendarId(calendarId)
                .orElseThrow(() -> new CalendarNotFoundException(calendarId));

        List<Meeting> overlappingMeetings = meetingRepository.findOverlappingMeetingsByUserCalendar(userCalendar, startTime, endTime);

        // Filter out the meeting being updated
        if (excludeMeetingId != null) {
            overlappingMeetings = overlappingMeetings.stream()
    ...
    @Retryable(
            ...
            maxAttempts = 3,
            backoff = @Backoff(delay = 1000, multiplier = 2))
    private List<Map<String, Object>> getExternalEvents(UUID calendarId, LocalDateTime from, LocalDateTime to) {
        try {
            String url = String.format("%s/api/events/calendar/%s/timerange?start=%s&end=%s",
